        ...
        HStack(alignment: .top, spacing: theme.spacing.small) {
            VStack(spacing: 0) {
                ...
                Rectangle()
                    ...
                    .frame(width: theme.chart.timelineLine, height: 34)
            }
...
struct PinesGlobalErrorBanner: View {
    @Environment(\.pinesTheme) private var theme
    let message: String
    let dismiss: () -> Void
    ...
    var body: some View {
        ...
        .background(theme.colors.dangerSoft, in: RoundedRectangle(cornerRadius: theme.radius.panel, style: .continuous))
        .overlay {
            RoundedRectangle(cornerRadius: theme.radius.panel, style: .continuous)
                .strokeBorder(theme.colors.danger.opacity(0.28), lineWidth: theme.stroke.hairline)
        }
        .accessibilityElement(children: .contain)
        .accessibilityIdentifier("pines.error.global")
    }
}
...
struct PinesThemePreviewCard: View {
    ...
    var body: some View {
        ...
        .frame(
            maxWidth: .infinity,
            minHeight: PinesThemePickerLayout.cardHeight,
            maxHeight: dynamicTypeSize.isAccessibilitySize ? nil : PinesThemePickerLayout.cardHeight,
...
struct PinesWorkspaceSwitcher: View {
    ...
    private func pickerLabel(for item: PinesWorkspaceSwitcherItem) -> some View {
        ...
        return HStack(alignment: .top, spacing: theme.spacing.xsmall) {
...
private struct PinesThemedFormModifier: ViewModifier {
    @Environment(\.pinesTheme) private var theme

    func body(content: Content) -> some View {
        content
            .scrollContentBackground(.hidden)
            .background(theme.colors.sheetBackground)
            .tint(theme.colors.accent)
    }
}
...
private struct PinesAdaptiveTextModifier: ViewModifier {
    @Environment(\.dynamicTypeSize) private var dynamicTypeSize
    let lines: Int
...
extension View {
    ...
    func pinesThemedForm() -> some View {
    ...
    func pinesFittingText(lines: Int = 1, minimumScale: CGFloat = 0.78) -> some View {
        modifier(PinesAdaptiveTextModifier(lines: lines, minimumScale: minimumScale))
    }
...
#if DEBUG
#Preview("Cloud Provider Components") {
    ScrollView {
        VStack(alignment: .leading, spacing: 12) {
            PinesCapabilityRow(
                title: "Responses and Tools",
                detail: "Provider supports stateful responses, hosted tools, and schema validation after probing.",
                systemImage: "text.bubble",
                status: .supported,
                secondaryStatus: .needsValidation,
                metricItems: [
                    .init("Context", value: "128k", systemImage: "text.word.spacing"),
                    .init("Tools", value: "hosted", systemImage: "wrench.and.screwdriver", tone: .warning)
                ]
            )

            PinesMetricPillGroup(items: [
                .init("Input", value: "12.4k", systemImage: "arrow.down.doc"),
                .init("Output", value: "1.8k", systemImage: "arrow.up.doc", tone: .info),
                .init("Cost", value: "$0.21 est.", systemImage: "creditcard", tone: .warning)
            ])

            HStack(spacing: 6) {
                PinesStatusChip(status: .enabled)
                PinesStatusChip(status: .accountGated)
                PinesProviderStorageBadge(kind: .providerHosted)
            }

            PinesToolTimelineRow(
                title: "Web search",
                provider: "OpenAI",
                toolType: "Hosted tool",
                status: .complete,
                inputSummary: "Search query approved for provider-hosted execution.",
                outputSummary: "Returned 4 cited sources and one generated artifact.",
                environmentLabel: "OpenAI hosted",
                metricItems: [
                    .init("Latency", value: "1.2s", systemImage: "timer"),
                    .init("Cost", value: "$0.03", systemImage: "creditcard", tone: .warning)
                ],
                systemImage: "globe"
            )

            PinesRunProvenancePanel(
                subtitle: "Request, route, usage, and storage state for the selected run.",
                fields: [
                    .init("Provider", "OpenRouter", systemImage: "cloud"),
                    .init("Model", "openai/gpt-4.1", systemImage: "cpu"),
                    .init("Request ID", "req_1234", systemImage: "number")
                ],
                metricItems: [
                    .init("Input", value: "8.1k", systemImage: "arrow.down.doc"),
                    .init("Output", value: "928", systemImage: "arrow.up.doc", tone: .info)
                ],
                storageKinds: [.inlineThisTurn, .cachedContext],
                status: .complete
            )

            PinesArtifactRow(
                title: "analysis.csv",
                subtitle: "Generated by provider-hosted code execution.",
                provider: "Anthropic",
                type: "CSV",
                status: .complete,
                storageKind: .providerHosted,
                metricItems: [
                    .init("Size", value: "42 KB", systemImage: "doc"),
                    .init("Linked", value: "1 run", systemImage: "link", tone: .info)
                ],
    ...
    .frame(width: 360)
    .pinesTheme(PinesTheme.resolve(template: .evergreen, mode: .light, systemScheme: .light))
    .pinesAppBackground()
}
#endif
