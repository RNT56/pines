...
#if canImport(MLXLMCommon) && canImport(PinesHubXetSupport) && canImport(Tokenizers)
import PinesHubXetSupport
import MLXLMCommon
import Tokenizers

enum PinesHuggingFaceBridgeError: LocalizedError {
...
struct PinesHubDownloader: MLXLMCommon.Downloader, @unchecked Sendable {
    ...
    func download(
        ...
        guard let repoID = HuggingFace.Repo.ID(rawValue: id) else {
            throw PinesHuggingFaceBridgeError.invalidRepositoryID(id)
        }
...
struct PinesTokenizerLoader: MLXLMCommon.TokenizerLoader {
    func load(from directory: URL) async throws -> any MLXLMCommon.Tokenizer {
        let upstream = try await Tokenizers.AutoTokenizer.from(modelFolder: directory)
        return PinesTokenizerBridge(upstream)
    }
}
