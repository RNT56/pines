...
The iOS job uses the `macos-26` GitHub-hosted runner and verifies the full iOS and watchOS build destinations needed by the `Pines` and `PinesWatch` schemes before running project validation. SDK visibility from `xcodebuild -showsdks` is not sufficient for the generic device builds on hosted runners, so the workflow installs missing platform payloads only when the actual scheme destinations are unavailable. Keep local XcodeGen at `2.45.4` or newer before regenerating `Pines.xcodeproj`.
