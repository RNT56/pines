...
- Shared provider lifecycle records, repositories, and previews, plus an Artifact Desk for generated images, video, speech, and research reports. A unified canvas, outcome command deck, compact work queue, and responsive detail inspector replace nested category and task navigation; provider-hosted file and vector-store management remains separate in Vault.
