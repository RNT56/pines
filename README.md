...
OpenRouter is treated as a routing platform rather than a generic text endpoint: its settings can constrain upstream provider order or allow/deny lists, fallbacks, parameter support, data collection, and zero-data-retention eligibility. Structured requests carry the schema and require a route that supports it; route and cost provenance remain an active hardening area.
