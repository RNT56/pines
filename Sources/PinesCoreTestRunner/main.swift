...
struct PinesCoreTestRunner {
    ...
    private static func testPersistenceSchema() throws {
        ...
        try expect(sql.contains("CREATE TABLE IF NOT EXISTS vault_embedding_profiles"), "missing vault embedding profiles table")
        try expect(sql.contains("CREATE TABLE IF NOT EXISTS vault_embedding_jobs"), "missing vault embedding jobs table")
        try expect(sql.contains("CREATE TABLE IF NOT EXISTS vault_retrieval_events"), "missing vault retrieval events table")
        ...
        try expect(sql.contains("CREATE TABLE IF NOT EXISTS provider_files"), "missing generic provider files table")
        try expect(sql.contains("CREATE TABLE IF NOT EXISTS provider_artifacts"), "missing generic provider artifacts table")
        try expect(sql.contains("CREATE TABLE IF NOT EXISTS provider_caches"), "missing generic provider caches table")
        try expect(sql.contains("CREATE TABLE IF NOT EXISTS provider_batches"), "missing generic provider batches table")
        ...
        try expect(sql.contains("INSERT OR IGNORE INTO provider_live_sessions"), "missing OpenAI realtime session migration copy")
        try expect(sql.contains("INSERT OR IGNORE INTO provider_structured_outputs"), "missing OpenAI structured output migration copy")
        ...
        try expectEqual(PinesDatabaseSchema.currentVersion, 20)
