...
Checked on 2026-05-18 against the Pines codebase, the pinned Schtack/RNT56 MLX forks, and current Hugging Face Hub metadata.
...
- `MLXSwiftLM`: `https://github.com/RNT56/mlx-swift-lm` at `c596b40cf3ac831f26006ee046dbabbb580b7c3b`

Pines status terms:

- `verified`: present in `CuratedModelManifest`; Pines enables TurboQuant by default for these installs.
...
| `mlx-community/Qwen3.5-0.8B-OptiQ-4bit` | Small chat | `qwen3_5` | ~0.67 GB | compact | installable | Latest small Qwen3.5 OptiQ candidate; Apache-2.0. |
| `mlx-community/Qwen3.5-2B-OptiQ-4bit` | Small reasoning/chat | `qwen3_5` | ~1.55 GB | compact | installable | Best newer compact Qwen3.5 candidate. |
| `mlx-community/Qwen3.5-4B-OptiQ-4bit` | Balanced chat | `qwen3_5` | ~3.29 GB | balanced | installable | Stronger Qwen3.5 candidate for healthy-memory phones. |
| `mlx-community/gemma-4-e2b-it-OptiQ-4bit` | Gemma chat | `gemma4` | ~4.33 GB | pro | installable | Newer Gemma 4 instruction candidate; Gemma license. |
| `mlx-community/gemma-4-e4b-it-OptiQ-4bit` | Larger Gemma chat | `gemma4` | ~6.57 GB | max | installable | Restrict to high-memory iPad/future/max tier. |
| `mlx-community/gemma-3-1b-it-qat-4bit` | Small Gemma chat | `gemma3_text` | ~0.77 GB | compact | installable | Good compact instruction-tuned Gemma option. |
| `mlx-community/gemma-3n-E2B-it-lm-4bit` | Gemma 3n chat | `gemma3n` | ~2.55 GB | compact edge | installable | Fits compact limit narrowly; smoke-test under memory pressure. |
...
| `mlx-community/Llama-3.2-3B-Instruct-4bit` | Explicit `Instruct` | `llama` | ~1.82 GB | compact | installable | Stronger Llama instruction option. |
...
| `mlx-community/gemma-3-1b-it-qat-4bit` | Explicit `it` | `gemma3_text` | ~0.77 GB | compact | installable | Good compact Gemma instruction model. |
| `mlx-community/gemma-3n-E2B-it-lm-4bit` | Explicit `it` | `gemma3n` | ~2.55 GB | compact edge | installable | Useful newer Gemma option; memory margin is thinner. |
| `mlx-community/gemma-4-e2b-it-OptiQ-4bit` | Explicit `it` | `gemma4` | ~4.33 GB | pro | installable | Best newer Gemma instruction candidate for pro devices. |
| `mlx-community/gemma-4-e4b-it-OptiQ-4bit` | Explicit `it` | `gemma4` | ~6.57 GB | max | installable | Only for high-memory iPad/future/max tier. |
| `mlx-community/Qwen3.5-0.8B-OptiQ-4bit` | Conversational/chat template | `qwen3_5` | ~0.67 GB | compact | installable | Latest tiny Qwen3.5 chat-style candidate. |
| `mlx-community/Qwen3.5-2B-OptiQ-4bit` | Conversational/chat template | `qwen3_5` | ~1.55 GB | compact | installable | Best newer compact chat-style candidate. |
| `mlx-community/Qwen3.5-4B-OptiQ-4bit` | Conversational/chat template | `qwen3_5` | ~3.29 GB | balanced | installable | Best newer balanced chat-style candidate. |
...
5. Keep non-curated models as `installable` until on-device generation and cancellation/memory-pressure behavior are verified.
...
- Pinned MLX Swift LM compatibility docs: https://github.com/RNT56/mlx-swift-lm/blob/c596b40cf3ac831f26006ee046dbabbb580b7c3b/Libraries/MLXLMCommon/Documentation.docc/model-compatibility.md
